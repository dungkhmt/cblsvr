...
import localsearch.domainspecific.vehiclerouting.vrp.moves.ThreeOptMove6;
...
public class FirstImprovementThreeOptMove6Explorer implements INeighborhoodExplorer {
			...
			for (Point y = XR.next(x); y != null && XR.isClientPoint(y); y = XR.next(y)) {
				...
				for (Point z = XR.next(y); XR.isClientPoint(z); z = XR.next(z)) {
					...
					if (XR.checkPerformThreeOptMove(x, y, z)) {
						...
						if (eval.lt(bestEval)){
							N.add(new ThreeOptMove6(mgr, eval, x, y, z));
						}
					}
				}
			}
