...
import localsearch.domainspecific.vehiclerouting.vrp.moves.TwoOptMove7;
...
public class FirstImprovementTwoOptMove7Explorer implements INeighborhoodExplorer {
				...
				for (Point x = XR.next(XR.getStartingPointOfRoute(i)); XR.isClientPoint(x); x = XR.next(x)) {
					...
					for (Point y = XR.next(XR.getStartingPointOfRoute(j)); XR.isClientPoint(y); y = XR.next(y)) {
						...
						if (XR.checkPerformTwoOptMove(x, y)) {
							...
							if (eval.lt(bestEval)){
								N.add(new TwoOptMove7(mgr, eval, x, y));
							}
						}
					}
				}
