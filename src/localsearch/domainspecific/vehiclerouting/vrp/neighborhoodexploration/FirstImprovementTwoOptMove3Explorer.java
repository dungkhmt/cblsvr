...
import localsearch.domainspecific.vehiclerouting.vrp.moves.TwoOptMove3;
...
public class FirstImprovementTwoOptMove3Explorer implements INeighborhoodExplorer {
				...
				for (Point x = XR.next(XR.getStartingPointOfRoute(i)); XR.isClientPoint(x); x = XR.next(x)) {
					...
					for (Point y = XR.next(XR.getStartingPointOfRoute(j)); XR.isClientPoint(y); y = XR.next(y)) {
						...
						if (XR.checkPerformTwoOptMove(x, y)) {
							...
							if (eval.lt(bestEval)){
								N.add(new TwoOptMove3(mgr, eval, x, y));
														}
						}
					}
				}
