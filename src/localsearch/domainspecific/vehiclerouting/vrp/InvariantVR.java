...
public interface InvariantVR {
	 ...
	 * implementing classes implement this method for initializing the data structure maPointained
	 */
	public void initPropagation();

	
    // move of type a [Groer et al., 2010]
    // move customer x to from route of x to route of y; insert x Pointo the position between y and next[y]
...
/*    
    // new proposed moves
    // x1, y1 are on route k1, x1 is before y1
    // x2, y2 are on route k2 (k2 != k1), x2 is before y2 
    // remove x1, y1 from route k1
    // insert x1 between x2 and next[x2]
    // insert y1 between y2 and next[y2]
    
    void propagateTwoPoPointsDifferentRouteMove(Point x1, Point y1, Point x2, Point y2);
    
    // new proposed moves
    // x1, y1, z1, t1 are on route k1
    // x2, y2, z2, t2 are on route k2
    // k1 != k2
    // x1, y1, x2, y2, z1, t1, z2, t2 are alldifferent
    // remove x1, y1 from route k1
    // remove x2, y2 from route k2
    // insert x1 between z2 and next[z2]
    // insert y1 between t2 and next[t2]
    // insert x2 between z1 and next[z1]
    // insert y2 between t1 and next[t1]
    
    void propagateTwoPoPointsExchangeMove(Point x1, Point y1, Point x2, Point y2, Point z1, Point t1, Point z2, Point t2);
...
*/    
    ...
    public void propagateFourPointsMove(Point x1, Point x2, Point x3, Point x4, Point y1, Point y2, Point y3, Point y4);
