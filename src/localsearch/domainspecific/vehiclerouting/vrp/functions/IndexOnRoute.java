...
public class IndexOnRoute implements IFunctionVR {
	...
	// compute the sum of demands from points s -> t on the route containing s and t
	...
	public double evaluateTwoOptMove1(Point x, Point y) {
		...
		if (XR.route(x) == XR.route(v)) {
			...
			if (XR.isBefore(x, v)) {//XR.index(v] > XR.index(x]) {
				return calc(v, XR.getTerminatingPointOfRoute(XR.route(x))) - XR.index(v) - 1;
			}
			return 0;
		}
		if (XR.route(y) == XR.route(v)) {
			if (v == XR.getTerminatingPointOfRoute(XR.route(y))) {
				return calc(XR.next(x), XR.getTerminatingPointOfRoute(XR.route(x))) +
					   calc(XR.next(y), XR.getTerminatingPointOfRoute(XR.route(y))) - 1 - XR.index(v);
			}
			if (XR.isBefore(y, v)) {//XR.index(v] > XR.index(y]) {
				double newValue = 0;
				newValue += calc(XR.next(x), XR.getTerminatingPointOfRoute(XR.route(x)));
				newValue += calc(XR.next(y), v);
				return newValue - XR.index(v) - 1;
			}
			//System.out.println(XR.index(x] + " + " + calc(y, v) + " - " + XR.index(v]);
	...
	public double evaluateTwoOptMove2(Point x, Point y) {
		...
		if (XR.route(y) == XR.route(v)) {
			if (v == XR.getTerminatingPointOfRoute(XR.route(y))) {
				return calc(XR.next(x), XR.getTerminatingPointOfRoute(XR.route(x))) +
					   calc(XR.next(y), XR.getTerminatingPointOfRoute(XR.route(y))) - XR.index(v) - 1;
			}
			if (XR.isBefore(y, v)) {//XR.index(v] > XR.index(y]) {
				double newValue = 0;
				newValue += calc(XR.next(x), XR.getTerminatingPointOfRoute(XR.route(x)));
	...
	public double evaluateTwoOptMove3(Point x, Point y) {
		...
		if (XR.route(x) == XR.route(v)) {
			...
			if (XR.isBefore(x, v)) {//XR.index(v] > XR.index(x]) {
				double newValue = 0;
				newValue += calc(XR.next(y), XR.getTerminatingPointOfRoute(XR.route(y)));
		...
		if (XR.route(y) == XR.route(v)) {
			...
			if (XR.isBefore(y, v)) {//XR.index(v] > XR.index(y]) {
				return calc(v, XR.getTerminatingPointOfRoute(XR.route(y))) - XR.index(v) - 1;
			}
			//System.out.println(XR.index(x] + " + " + calc(y, v) + " - " + XR.index(v]);
			return XR.index(x) + calc(v, y) - XR.index(v);
		}
	...
	public double evaluateTwoOptMove5(Point x, Point y) {
		...
		if (v == XR.getTerminatingPointOfRoute(XR.route(x))) {
			return XR.index(x) + calc(XR.next(y), XR.getTerminatingPointOfRoute(XR.route(y))) - XR.index(v);
		}
		if (v == XR.getTerminatingPointOfRoute(XR.route(y))) {
			return XR.index(y) + calc(XR.next(x), XR.getTerminatingPointOfRoute(XR.route(x))) - XR.index(v);
		}
		if (XR.route(x) == XR.route(v) && XR.isBefore(x, v)) {//XR.index(v] > XR.index(x]) {
			return XR.index(y) + calc(XR.next(x), v) - XR.index(v);
		}
		if (XR.route(y) == XR.route(v) && XR.isBefore(y, v)) {////XR.index(v] > XR.index(y]) {
	...
	public double evaluateTwoOptMove6(Point x, Point y) {
		...
		if (XR.route(x) == XR.route(v)) {
			...
			return calc(XR.next(y), XR.getTerminatingPointOfRoute(XR.route(y))) + calc(v, x) - XR.index(v) - 1;
		}
		if (XR.route(y) == XR.route(v) && XR.isBefore(y, v)) {//XR.index(v] > XR.index(y]) {
			return calc(v, XR.getTerminatingPointOfRoute(XR.route(y))) - XR.index(v) - 1;
		}
	...
	public double evaluateTwoOptMove7(Point x, Point y) {
		...
		if (XR.route(y) == XR.route(v)) {
			...
			return calc(XR.next(x), XR.getTerminatingPointOfRoute(XR.route(x))) + calc(v, y) - XR.index(v) - 1;
		}
		if (XR.route(x) == XR.route(v) && XR.isBefore(x, v)) {//XR.index(v] > XR.index(x]) {
			return calc(v, XR.getTerminatingPointOfRoute(XR.route(x))) - XR.index(v) - 1;
		}
	...
	public double evaluateOrOptMove1(Point x1, Point x2, Point y) {
		...
		if (XR.route(x1) == XR.route(v)) {
			if (XR.isBefore(x2, v)) {//XR.index(v] > XR.index(x2]) {
				return XR.index(XR.prev(x1)) + calc(XR.next(x2), v) - XR.index(v);
			}
	...
	public double evaluateThreeOptMove1(Point x, Point y, Point z) {
		...
		if (XR.route(x) == XR.route(v)) {
			if (XR.isBefore(v, XR.next(z)) && XR.isBefore(y, v)) {////XR.index(v] <= XR.index(z] && XR.index(v] > XR.index(y]) {
				return XR.index(x) + calc(v, z) - XR.index(v);
			}
			if (XR.isBefore(v, XR.next(y)) && XR.isBefore(x, v)) {//XR.index(v] <= XR.index(y] && XR.index(v] > XR.index(x]) {
				return XR.index(x) + calc(XR.next(y), z) + calc(XR.next(x), v) - XR.index(v);
			}
		}
		return 0;
	}
	...
	public double evaluateThreeOptMove2(Point x, Point y, Point z) {
		// TODO Auto-generated method stub
		if (!XR.checkPerformThreeOptMove(x, y, z)) {
			System.out.println(name() + ":: Error evaluateThreOptMove2: " + x + " " + y + " " + z + "\n" + XR.toString());
    		System.exit(-1);
		}
		if (v == XR.getStartingPointOfRoute(XR.route(x)) || v == XR.getTerminatingPointOfRoute(XR.route(x))) {
			return 0;
	...
	public double evaluateThreeOptMove3(Point x, Point y, Point z) {
		...
		if (XR.route(x) == XR.route(v)) {
			...
			if (XR.isBefore(y, v)) {//if (XR.index(v] > XR.index(y]) {
				return XR.index(x) + calc(XR.next(x), y) + calc(v, z) - XR.index(v);
			}
	...
	public double evaluateThreeOptMove7(Point x, Point y, Point z) {
		...
		if (XR.route(x) == XR.route(v)) {
			...
			if (XR.isBefore(x, v)) {//if (XR.index(v] > XR.index(x]) {
				return XR.index(x) + calc(XR.next(y), z) + calc(y, v) - XR.index(v);
			}
		}
		return 0;
	}
	...
	public double evaluateThreeOptMove8(Point x, Point y, Point z) {
		if (!XR.checkPerformThreeOptMove(x, y, z)) {
			System.out.println(name() + ":: Error evaluatThreeOptMove8: " + x + " " + y + " " + z + "\n" + XR.toString());
    		System.exit(-1);
		}
		if (v == XR.getStartingPointOfRoute(XR.route(x)) || v == XR.getTerminatingPointOfRoute(XR.route(x))) {
			return 0;
		...
		if (XR.route(x) == XR.route(v)) {
			...
			if (XR.isBefore(y, v)) {//if (XR.index(v] > XR.index(y]) {
				double newV = calc(z, v) + calc(y, XR.next(x));
				newV += calc(XR.next(z), XR.getTerminatingPointOfRoute(XR.route(x))) - 1;
	...
	public double evaluateCrossExchangeMove(Point x1, Point y1, Point x2, Point y2) {
		...
		if (XR.route(x1) == XR.route(v)) {
			if (XR.isBefore(y1, v)) {//if (XR.index(v] > XR.index(y1]) {
				return XR.index(x1) + calc(XR.next(x2), y2) + calc(XR.next(y1), v) - XR.index(v);
			}
		...
		if (XR.route(x2) == XR.route(v)) {
			if (XR.isBefore(y2, v)) {//if (XR.index(v] > XR.index(y2]) {
				return XR.index(x2) + calc(XR.next(x1), y1) + calc(XR.next(y2), v) - XR.index(v);
			}
...
//			while (!XR.checkPerformThreePointsMove(p[x1], p[x2], p[x3], p[y1], p[y2], p[y3])) {
//				x1 = rand.nextInt(N);
//				y1 = rand.nextInt(N);
//				x2 = rand.nextInt(N);
//				y2 = rand.nextInt(N);
...
//			for (int i = 0; i < N; i++) {
//				delta[i] = f[i].evaluateThreePointsMove(p[x1], p[x2], p[x3], p[y1], p[y2], p[y3]);
//			}
//			mgr.performThreePointsMove(p[x1], p[x2], p[x3], p[y1], p[y2], p[y3]);
//			for (int i = 0; i < N; i++) {
//				newV[i] = f[i].getValue();
//			}
