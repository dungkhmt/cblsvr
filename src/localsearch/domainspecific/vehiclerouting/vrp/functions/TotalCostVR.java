...
public class TotalCostVR implements IFunctionVR {
	...
	private void update(int k) {
        ...
        for (Point u = sp; u != tp; u = XR.next(u)) {
            costRight[getIndex(XR.next(u))] = costRight[getIndex(u)] + awm.getWeight(u, XR.next(u)); 
        }
        costLeft[getIndex(tp)] = 0;
        for (Point u = tp; u != sp; u = XR.prev(u)) {
            costLeft[getIndex(XR.prev(u))] = costLeft[getIndex(u)] + awm.getWeight(u, XR.prev(u));
        }
	}
	...
	public void propagateTwoOptMove1(Point x, Point y) {
		// TODO Auto-generated method stub
		double oldX = getCostRight(XR.getTerminatingPointOfRoute(XR.oldRoute(x)));
        double oldY = getCostRight(XR.getTerminatingPointOfRoute(XR.oldRoute(y)));
        update(XR.oldRoute(x));
        update(XR.oldRoute(y));
        double newX = getCostRight(XR.getTerminatingPointOfRoute(XR.oldRoute(x)));
        double newY = getCostRight(XR.getTerminatingPointOfRoute(XR.oldRoute(y)));
	...
	public void propagateOrOptMove1(Point x1, Point x2, Point y) {
		// TODO Auto-generated method stub
		double oldX = getCostRight(XR.getTerminatingPointOfRoute(XR.oldRoute(x1)));
        double oldY = getCostRight(XR.getTerminatingPointOfRoute(XR.oldRoute(y)));
        update(XR.oldRoute(x1));
        update(XR.oldRoute(y));
        double newX = getCostRight(XR.getTerminatingPointOfRoute(XR.oldRoute(x1)));
        double newY = getCostRight(XR.getTerminatingPointOfRoute(XR.oldRoute(y)));
        value += newX + newY - oldX - oldY;
	}
	...
	public void propagateThreeOptMove1(Point x, Point y, Point z) {
		// TODO Auto-generated method stub
        double oldV = getCostRight(XR.getTerminatingPointOfRoute(XR.oldRoute(y)));
        update(XR.oldRoute(y));
        double newV = getCostRight(XR.getTerminatingPointOfRoute(XR.oldRoute(y)));
        value += newV - oldV;
	}
	...
	public void propagateCrossExchangeMove(Point x1, Point y1, Point x2,
		...
		// TODO Auto-generated method stub
		double oldX = getCostRight(XR.getTerminatingPointOfRoute(XR.oldRoute(x1)));
        double oldY = getCostRight(XR.getTerminatingPointOfRoute(XR.oldRoute(x2)));
        update(XR.oldRoute(x1));
        update(XR.oldRoute(x2));
        double newX = getCostRight(XR.getTerminatingPointOfRoute(XR.oldRoute(x1)));
        double newY = getCostRight(XR.getTerminatingPointOfRoute(XR.oldRoute(x2)));
        value += newX + newY - oldX - oldY;
	}
	...
	public double evaluateOnePointMove(Point x, Point y) {
		...
		for (int i = 0; i < n; i++) {
			...
			eval += getCost(prev, next) - getCost(prev, xx[i]) - getCost(xx[i], next);
			V.add(xx[i]);
		}
		...
		for (int i = 0; i < n; i++) {
			...
			eval += getCost(prev, xx[i]) + getCost(xx[i], next) - getCost(prev, next);
			M.put(yy[i], xx[i]);
		}
		return eval;
	}
	...
	public double evaluateTwoOptMove1(Point x, Point y) {
        ...
        double newX = getCostRight(x) + getCost(x, y) 
        		+ calc(y, XR.next(XR.getStartingPointOfRoute(XR.route(y))))
        		+ getCost(XR.next(XR.getStartingPointOfRoute(XR.route(y))), XR.getTerminatingPointOfRoute(XR.route(x)));
        double newY = 0;
        if (XR.next(x) == XR.getTerminatingPointOfRoute(XR.route(x))) {
        	newY = getCost(XR.getStartingPointOfRoute(XR.route(y)), XR.next(y));
        } else {
        	newY = getCost(XR.getStartingPointOfRoute(XR.route(y)), XR.prev(XR.getTerminatingPointOfRoute(XR.route(x))))
        			+ calc(XR.prev(XR.getTerminatingPointOfRoute(XR.route(x))), XR.next(x))
        			+ getCost(XR.next(x), XR.next(y));
        }
	...
	public double evaluateTwoOptMove2(Point x, Point y) {
        ...
        double oldX = getCostRight(XR.getTerminatingPointOfRoute(XR.route(x)));
        double oldY = getCostRight(XR.getTerminatingPointOfRoute(XR.route(y)));
        double newX = getCost(XR.getStartingPointOfRoute(XR.route(x)), XR.next(XR.getStartingPointOfRoute(XR.route(y))))
        		+ calc(XR.next(XR.getStartingPointOfRoute(XR.route(y))), y)
        		+ getCost(y, x) + calc(x, XR.next(XR.getStartingPointOfRoute(XR.route(x))))
        		+ getCost(XR.next(XR.getStartingPointOfRoute(XR.route(x))), XR.getTerminatingPointOfRoute(XR.route(x)));
        double newY = 0;
        if (XR.next(x) == XR.getTerminatingPointOfRoute(XR.route(x))) {
        	newY = getCost(XR.getStartingPointOfRoute(XR.route(y)), XR.next(y));
        } else {
	...
	public double evaluateTwoOptMove3(Point x, Point y) {
        ...
        if (XR.next(x) != XR.getTerminatingPointOfRoute(XR.route(x))) {
        	if (XR.next(y) == XR.getTerminatingPointOfRoute(XR.route(y))) {
            	newY = getCost(XR.getStartingPointOfRoute(XR.route(y)), XR.next(x));
            } else {
            	newY = getCost(XR.getStartingPointOfRoute(XR.route(y)), XR.prev(XR.getTerminatingPointOfRoute(XR.route(y))))
            			+ calc(XR.prev(XR.getTerminatingPointOfRoute(XR.route(y))), XR.next(y))
            			+ getCost(XR.next(y), XR.next(x));
            }
        	newY += calc(XR.next(x), XR.prev(XR.getTerminatingPointOfRoute(XR.route(x)))) 
        			+ getCost(XR.prev(XR.getTerminatingPointOfRoute(XR.route(x))), XR.getTerminatingPointOfRoute(XR.route(y)));
        } else {
        	if (XR.next(y) == XR.getTerminatingPointOfRoute(XR.route(y))) {
        		newY = getCost(XR.getStartingPointOfRoute(XR.route(y)), XR.getTerminatingPointOfRoute(XR.route(y)));
        	} else {
            	newY = getCost(XR.getStartingPointOfRoute(XR.route(y)), XR.prev(XR.getTerminatingPointOfRoute(XR.route(y))))
            			+ calc(XR.prev(XR.getTerminatingPointOfRoute(XR.route(y))), XR.next(y))
            			+ getCost(XR.next(y), XR.getTerminatingPointOfRoute(XR.route(y)));
        	}
        }
	...
	public double evaluateTwoOptMove5(Point x, Point y) {
        ...
        double newX = 0;
        if (XR.next(y) == XR.getTerminatingPointOfRoute(XR.route(y))) {
        	newX = getCostRight(x) + getCost(x, XR.getTerminatingPointOfRoute(XR.route(x)));
        } else {
        	newX = getCostRight(x) + getCost(x, XR.next(y)) 
        			+ calc(XR.next(y), XR.prev(XR.getTerminatingPointOfRoute(XR.route(y))))
        			+ getCost(XR.prev(XR.getTerminatingPointOfRoute(XR.route(y))), XR.getTerminatingPointOfRoute(XR.route(x)));
        }
        double newY = 0;
        if (XR.next(x) == XR.getTerminatingPointOfRoute(XR.route(x))) {
        	newY = getCostRight(y) + getCost(y, XR.getTerminatingPointOfRoute(XR.route(y)));
        } else {
        	newY = getCostRight(y) + getCost(y, XR.next(x)) 
        			+ calc(XR.next(x), XR.prev(XR.getTerminatingPointOfRoute(XR.route(x))))
        			+ getCost(XR.prev(XR.getTerminatingPointOfRoute(XR.route(x))), XR.getTerminatingPointOfRoute(XR.route(y)));
        }
        return newX + newY - oldX - oldY;
	}
	...
	public double evaluateTwoOptMove6(Point x, Point y) {
        ...
        if (XR.next(y) == XR.getTerminatingPointOfRoute(XR.route(y))) {
        	...
        	newX = getCost(XR.getStartingPointOfRoute(XR.route(x)), XR.prev(XR.getTerminatingPointOfRoute(XR.route(y))))
        			+ calc(XR.prev(XR.getTerminatingPointOfRoute(XR.route(y))), XR.next(y)) 
        			+ getCost(XR.next(y), x)
        			+ calc(x, XR.next(XR.getStartingPointOfRoute(XR.route(x))))
        			+ getCost(XR.next(XR.getStartingPointOfRoute(XR.route(x))), XR.getTerminatingPointOfRoute(XR.route(x)));
	...
	public double evaluateTwoOptMove8(Point x, Point y) {
        ...
        if (XR.next(x) == XR.getTerminatingPointOfRoute(XR.route(x))) {
        	newY = getCost(XR.getStartingPointOfRoute(XR.route(y)), y) 
        			+ calc(y, XR.next(XR.getStartingPointOfRoute(XR.route(y))))
        			+ getCost(XR.next(XR.getStartingPointOfRoute(XR.route(y))), XR.getTerminatingPointOfRoute(XR.route(y)));
        } else {
        	newY = getCost(XR.getStartingPointOfRoute(XR.route(y)), XR.prev(XR.getTerminatingPointOfRoute(XR.route(x))))
        			+ calc(XR.prev(XR.getTerminatingPointOfRoute(XR.route(x))), XR.next(x)) 
        			+ getCost(XR.next(x), y)
	...
	public double evaluateOrOptMove1(Point x1, Point x2, Point y) {
        ...
        double oldY = getCostRight(XR.getTerminatingPointOfRoute(XR.route(y)));
        double newX = getCostRight(XR.prev(x1)) + getCost(XR.prev(x1), XR.next(x2)) + calc(XR.next(x2), XR.getTerminatingPointOfRoute(XR.route(x1)));
        double newY = getCostRight(y) + getCost(y, x1) + calc(x1, x2) + getCost(x2, XR.next(y)) + calc(XR.next(y), XR.getTerminatingPointOfRoute(XR.route(y)));
        return newX + newY - oldX - oldY;
	}
	...
	public double evaluateOrOptMove2(Point x1, Point x2, Point y) {
        ...
        double oldY = getCostRight(XR.getTerminatingPointOfRoute(XR.route(y)));
        double newX = getCostRight(XR.prev(x1)) + getCost(XR.prev(x1), XR.next(x2)) + calc(XR.next(x2), XR.getTerminatingPointOfRoute(XR.route(x1)));
        double newY = getCostRight(y) + getCost(y, x2) + calc(x2, x1) + getCost(x1, XR.next(y)) + calc(XR.next(y), XR.getTerminatingPointOfRoute(XR.route(y)));
        return newX + newY - oldX - oldY;
	}
	...
	public double evaluateThreeOptMove2(Point x, Point y, Point z) {
		...
		if (XR.next(z) != XR.getTerminatingPointOfRoute(XR.route(x))) {
			d1 = getCost(XR.getStartingPointOfRoute(XR.route(x)), XR.prev(XR.getTerminatingPointOfRoute(XR.route(x))))
					- getCost(XR.getTerminatingPointOfRoute(XR.route(x)), XR.prev(XR.getTerminatingPointOfRoute(XR.route(x))))
					;
	...
	public double evaluateCrossExchangeMove(Point x1, Point y1, Point x2,
        ...
        double newX1 = getCost(x1, XR.next(x2)) + calc(XR.next(x2), y2) + getCost(y2, XR.next(y1));
        double newX2 = getCost(x2, XR.next(x1)) + calc(XR.next(x1), y1) + getCost(y1, XR.next(y2));
	...
	public double evaluateKPointsMove(ArrayList<Point> x, ArrayList<Point> y) {
		...
		for (int i = 0; i < n; i++) {
			if (XR.route(x.get(i)) != Constants.NULL_POINT) {
				...
				eval += getCost(prev, next) - getCost(prev, x.get(i)) - getCost(x.get(i), next);
				V.add(x.get(i));
			}
		}
		HashMap<Point, Point> M = new HashMap<Point, Point>();
		for (int i = 0; i < n; i++) {
			if (y.get(i) != CBLSVR.NULL_POINT) {
				...
				eval += getCost(prev, x.get(i)) + getCost(x.get(i), next) - getCost(prev, next);
...
//			while (!XR.checkPerformFourPointsMove(p[x1], p[x2], p[x3], p[x4], p[y1], p[y2], p[y3], p[y4])) {
//				x1 = rand.nextInt(N);
//				y1 = rand.nextInt(N);
//				x2 = rand.nextInt(N);
//				y2 = rand.nextInt(N);
...
//			//for (int i = 0; i < N; i++) {
//				delta = f.evaluateFourPointsMove(p[x1], p[x2], p[x3], p[x4], p[y1], p[y2], p[y3], p[y4]);
//			//}
//			mgr.performFourPointsMove(p[x1], p[x2], p[x3], p[x4], p[y1], p[y2], p[y3], p[y4]);
//			//for (int i = 0; i < N; i++) {
//				newV = f.getValue();
//			//}
