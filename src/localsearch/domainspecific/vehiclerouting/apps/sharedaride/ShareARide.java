...
public class ShareARide {
	...
	public ShareARide(Info info) {
		...
		for (int i = 1; i <= info.nbVehicle; ++i) {
			...
			Point sp = new Point(startPointId, info.p[startPointId].getX(),
					info.p[startPointId].getY());
			int stopPointId = i + info.nRequest * 2 + info.nbVehicle;
			Point tp = new Point(stopPointId, info.p[stopPointId].getX(),
					info.p[stopPointId].getY());
			points.add(sp);
			points.add(tp);
			startPoints.add(sp);
			stopPoints.add(tp);

			earliestAllowedArrivalTime.put(sp,
					(int) (info.earliest[startPointId]));
			serviceDuration.put(sp, (int) (info.serviceTime[startPointId]));
			lastestAllowedArrivalTime.put(sp,
					(int) (info.lastest[startPointId]));

			earliestAllowedArrivalTime.put(tp,
					(int) (info.earliest[stopPointId]));
			serviceDuration.put(tp, (int) (info.serviceTime[stopPointId]));
			lastestAllowedArrivalTime
					.put(tp, (int) (info.lastest[stopPointId]));
		}

		for (int i = 0; i < info.nRequest; ++i) {
			Point pickup = new Point(i * 2 + 1, info.p[i * 2 + 1].getX(),
					info.p[i * 2 + 1].getY());
			Point delivery = new Point(2 * i + 2, info.p[2 * i + 2].getX(),
					info.p[2 * i + 2].getY());
	...
	public void stateModel() {
		...
		for (int i = 0; i < nRequest; ++i) {
			...
			/*
			 * IFunctionVR indexP = new IndexOnRoute(XR, pickup); IFunctionVR
			 * indexD = new IndexOnRoute(XR,delivery); IFunctionVR routeP = new
			 * RouteIndex(XR,pickup); IFunctionVR routeD = new
			 * RouteIndex(XR,delivery); IConstraintVR pBeforeD = new
			 * LeqFunctionFunction(indexP, indexD); S.post(new
			 * ScaleConstraint(pBeforeD,scale)); IConstraintVR pAndDInSameRoute
			 * = new EquFunctionFunction(routeP,routeD); S.post(new
			 * ScaleConstraint(pAndDInSameRoute, scale));
			 * if(type.get(i)==PEOPLE) { IFunctionVR disPAndDIndex = new
			 * MinusFunctionFunctionVR(indexD, indexP); IConstraintVR dIsNextOfP
			 * = new EquFunctionConstantVR(disPAndDIndex,1); //= new
			 * LeqFunctionConstant(disPAndDIndex, M); S.post(new
			 * ScaleConstraint(dIsNextOfP,scale)); }
			 */
		}

		IConstraintVR goodC = new CPickupDeliveryOfGoodVR(XR,
				pickup2DeliveryOfGood);
		IConstraintVR peopleC = new CPickupDeliveryOfPeopleVR(XR,
		...
		CEarliestArrivalTimeVR cEarliest = new CEarliestArrivalTimeVR(eat,
				lastestAllowedArrivalTime);
		S.post(cEarliest);
	...
	ArrayList<ArrayList<INeighborhoodExplorer>> search6(LexMultiFunctions F) {
		...
		NE = new ArrayList<>();
		NE.add(new GreedyOneRequestMoveExplorerLimit(XR, F, 2, pickupPoints,
				deliveryPoints));
		NE.add(new GreedyTwoRequestMoveExplorerLimit(XR, F, pickupPoints,
				deliveryPoints));
	...
	ArrayList<ArrayList<INeighborhoodExplorer>> search7(LexMultiFunctions F) {
		...
		NE = new ArrayList<>();
		NE.add(new GreedyTwoRequestMoveExplorerLimit(XR, F, 0.25, pickupPoints,
				deliveryPoints));
		NE.add(new GreedyOrOptMove1ExplorerLimit(XR, F, 3));
		listNE.add(NE);

		NE = new ArrayList<>();
		NE.add(new GreedyTwoRequestMoveExplorerLimit(XR, F, 1, pickupPoints,
				deliveryPoints));
		NE.add(new GreedyCrossExchangeMoveExplorerLimit(XR, F, 2, pickupPoints,
				deliveryPoints));
		NE.add(new GreedyOrOptMove1ExplorerLimit(XR, F, 5, pickupPoints,
				deliveryPoints));
		...
		NE.add(new GreedyOneRequestMoveExplorerLimit(XR, F, 2, pickupPoints,
				deliveryPoints));
		NE.add(new GreedyCrossExchangeMoveExplorerLimit(XR, F, 6, pickupPoints,
				deliveryPoints));
		NE.add(new GreedyOrOptMove1ExplorerLimit(XR, F, 10, pickupPoints,
				deliveryPoints));
		listNE.add(NE);
	...
	ArrayList<ArrayList<INeighborhoodExplorer>> search8(LexMultiFunctions F) {
		...
		NE.add(new GreedyTwoRequestMoveExplorerLimit(XR, F, 0.25, pickupPoints,
				deliveryPoints));
		NE.add(new GreedyOneRequestMoveExplorerLimit(XR, F, 0.1, 3,
		...
		NE.add(new GreedyOrOptMove1ExplorerLimit(XR, F, 5, pickupPoints,
				deliveryPoints));
		NE.add(new GreedyOneRequestMoveExplorerLimit(XR, F, 0.3, 3,
				pickupPoints, deliveryPoints));

		listNE.add(NE);

		NE = new ArrayList<>();
		NE.add(new GreedyOneRequestMoveExplorerLimit(XR, F, 0.6, 3,
				pickupPoints, deliveryPoints));
		NE.add(new GreedyCrossExchangeMoveExplorerLimit(XR, F, 6, pickupPoints,
				deliveryPoints));
		NE.add(new GreedyOrOptMove1ExplorerLimit(XR, F, 10, pickupPoints,
	...
	VarRoutesVR search(int maxIter, int timeLimit, int searchMethod) {
		...
		VariableNeighborhoodSearch vns = new VariableNeighborhoodSearch(mgr, F,
				listNE, pickupPoints, deliveryPoints);
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		for (int data = 1; data <= 10; ++data) {
			...
			PrintWriter out = new PrintWriter(new File("out/S" + S
					+ "/N100_R10_D" + data + "_S" + S + ".txt"));
